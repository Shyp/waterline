...
module.exports = function integrate(cache, joinInstructions, primaryKey, cb) {
  ...
  // Ensure valid usage
  var invalid = false;
  invalid = invalid || lusitania(cache).to({ type: 'object' });
  invalid = invalid || lusitania(joinInstructions).to({ type: 'array' });
  invalid = invalid || lusitania(joinInstructions[0]).to({ type: 'object' });
  invalid = invalid || lusitania(joinInstructions[0].parent).to({ type: 'string' });
  invalid = invalid || lusitania(cache[joinInstructions[0].parent]).to({ type: 'object' });
