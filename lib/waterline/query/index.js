...
Query.prototype.sync = function(cb) {
  ...
  if(this.migrate && ['drop', 'safe'].indexOf(this.migrate) > -1) {
  ...
  // Throw Error
  else cb(new Error('Invalid `migrate` strategy defined for collection. Must be one of the following: drop, safe'));
};
